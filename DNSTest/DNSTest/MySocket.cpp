...
bool MySocketServer::Receive(char receivebuf[],int length,int &receivelength)//从client客户端接收数据
{
	if(flag==TCP)
	{
...
bool MySocketServer::Send(const char sendbuf[],int length)//向client客户端发送数据(使用此函数前必须先使用Receive获取到client客户端的ip和端口号等信息)
{
...
bool MySocketServer::GetClientInfo()//获取client客户端的ip和端口号等信息
{
	strcpy(client_ip,inet_ntoa(addrclient.sin_addr));
	client_port=ntohs(addrclient.sin_port);
	return true;
...
bool MySocketServer::SetClientInfo()//设置client客户端的ip和端口号等信息
{
